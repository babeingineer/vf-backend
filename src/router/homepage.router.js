...
router.post("/", async (req, res) => {
    try{
        ...
        if(data) {
            res.send({message: "updated", data: await homepageModel.findOneAndUpdate(req.body)});
        }
        else res.send({message: "created", data: await homepageModel.create(req.body)});
    }
    catch(error){
...
router.post("/images", upload.array("images"), async (req, res) => {
    if(req.query.name == "slider_images") {
        ...
        res.send({message: "updated", data: await homepageModel.findOneAndUpdate(body)});
    }
